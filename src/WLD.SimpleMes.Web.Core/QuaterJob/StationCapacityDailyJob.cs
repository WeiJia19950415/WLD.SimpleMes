...
namespace WLD.SimpleMes.QuaterJob
{
    ...
    public class StationCapacityDailyJob : IJob, IParmerinfoProvider
    {
        private readonly IReportAppService _reportAppService;
        public string JobUniqName => "工序产能日统计任务—允许重跑";

        public StationCapacityDailyJob()
        {

        }

        public StationCapacityDailyJob(IReportAppService reportAppService)
        {
        ...
        public async Task Execute(IJobExecutionContext context)
        {
            ...
            while (staticTime.Date.AddDays(1).Date < DateTime.Now.Date)
            {
               ...
               await _reportAppService.BuildWorkProcessCapacityDailyReportAsync(staticTime, materialNumber);
            }
